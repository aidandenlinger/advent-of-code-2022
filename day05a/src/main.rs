...
impl Move {
    fn apply(&self, stacks: &mut HashMap<usize, Vec<char>>) {
        for _ in 0..self.amt {
            let popped = stacks.get_mut(&self.from).unwrap().pop().unwrap();
            stacks.get_mut(&self.to).unwrap().push(popped);
        }
    }
}
...
move 1 from 1 to 2";
    ...
    #[test]
    fn test_web() {
        assert_eq!("CMZ", run(WEB_INPUT))
    }

    #[test]
    fn test() {
        assert_eq!("VQZNJMWTR", run(INPUT))
    }
}
